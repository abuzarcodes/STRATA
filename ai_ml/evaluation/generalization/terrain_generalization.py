"""
Milestone 4 Evaluation Script: Terrain Slope Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Terrain Slope Generalization conditions.
"""
...
def evaluate_terrain():
    print("Running Terrain Slope Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4500)

    slopes = [0.0, 3.0, 5.0, 8.0, 10.0, 12.0, 15.0]
    results = {}
    for slope in slopes:
        key = f"{slope}deg"
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, terrain_slope_deg=slope, setback_m=2.0)
        results[key] = res
        print(f"  Slope {key:8s} | Base F1: {res['baseline']['f1_mean']:.4f} | ML F1: {res['ml_model']['f1_mean']:.4f}")
    return results
