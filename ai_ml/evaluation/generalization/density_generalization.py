"""
Milestone 4 Evaluation Script: Density Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Density Generalization conditions.
"""
...
def evaluate_density():
    print("Running Point Density Sweep Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4400)

    densities = [0.25, 0.50, 0.75, 0.92, 1.00, 1.50, 2.00, 3.00, 5.00]
    results = {}
    for d_val in densities:
        key = f"{d_val}pts_m2" + ("_AUCKLAND_LIKE" if d_val == 0.92 else "")
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, target_density_pts_per_sqm=d_val, setback_m=2.0)
        results[key] = res
        print(f"  Density {key:25s} | Base F1: {res['baseline']['f1_mean']:.4f} | ML F1: {res['ml_model']['f1_mean']:.4f}")
    return results
