"""
Milestone 4 Evaluation Script: Setback Sweep Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Setback Sweep Generalization conditions.
"""
...
def evaluate_setback():
    print("Running Controlled Setback Sweep Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4300)

    setbacks = [4.0, 3.5, 3.0, 2.5, 2.0, 1.5, 1.0, 0.5, 0.0]
    results = {}
    for s_val in setbacks:
        key = f"{s_val}m"
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, setback_m=s_val, attached=(s_val==0.0))
        results[key] = res
        print(f"  Setback {key:6s} | Base F1: {res['baseline']['f1_mean']:.4f} (Merge {res['baseline']['merge_mean']:.1f}%) | ML F1: {res['ml_model']['f1_mean']:.4f} (Merge {res['ml_model']['merge_mean']:.1f}%)")
    return results
