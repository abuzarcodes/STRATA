"""
Milestone 4 Evaluation Script: Multi-Building Parcels Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Multi-Building Parcels Generalization conditions.
"""
...
def evaluate_multi_building():
    print("Running Multi-Building Parcels (2 to 5 Buildings) Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4800)

    counts = [2, 3, 4, 5]
    results = {}
    for n_bldg in counts:
        key = f"{n_bldg}_buildings"
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, num_buildings=n_bldg, setback_m=1.5, scene_extent_m=100.0)
        results[key] = res
        print(f"  Parcels {key:15s} | Base F1: {res['baseline']['f1_mean']:.4f} | ML F1: {res['ml_model']['f1_mean']:.4f}")
    return results

if __name__ == "__main__":
    evaluate_multi_building()
