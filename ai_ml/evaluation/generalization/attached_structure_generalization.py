"""
Milestone 4 Evaluation Script: Attached Structures Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Attached Structures Generalization conditions.
"""
...
def evaluate_attached():
    print("Running Attached Structures (0.0m Setback) Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4700)

    cases = [
        {"name": "identical_rect_touching", "heights": [6.0, 6.0], "footprint_type": "rect"},
        {"name": "mixed_height_touching", "heights": [6.0, 18.0], "footprint_type": "rect"},
        {"name": "rotated_touching", "heights": [6.0, 12.0], "footprint_type": "rotated"},
        {"name": "lshape_touching", "heights": [6.0, 12.0], "footprint_type": "l_shape"}
    ]
    results = {}
    for c in cases:
        name = c["name"]
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, setback_m=0.0, attached=True, heights=c["heights"], footprint_type=c["footprint_type"])
        results[name] = res
        print(f"  Attached {name:25s} | Base F1: {res['baseline']['f1_mean']:.4f} (Merge {res['baseline']['merge_mean']:.1f}%) | ML F1: {res['ml_model']['f1_mean']:.4f} (Merge {res['ml_model']['merge_mean']:.1f}%)")
    return results
