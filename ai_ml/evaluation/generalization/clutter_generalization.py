"""
Milestone 4 Evaluation Script: Urban Clutter Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Urban Clutter Generalization conditions.
"""
...
def evaluate_clutter():
    print("Running Urban Clutter Generalization...")
    ...
    gen = ExtendedSceneGenerator(seed=4600)

    scenarios = [
        {"name": "clean", "has_clutter": False, "has_vegetation": False},
        {"name": "rooftop_clutter", "has_clutter": True, "has_vegetation": False},
        {"name": "vegetation_overhang", "has_clutter": False, "has_vegetation": True},
        {"name": "combined_clutter", "has_clutter": True, "has_vegetation": True}
    ]
    results = {}
    for sc in scenarios:
        name = sc["name"]
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, setback_m=2.0, has_clutter=sc["has_clutter"], has_vegetation=sc["has_vegetation"])
        results[name] = res
        print(f"  Clutter {name:20s} | Base F1: {res['baseline']['f1_mean']:.4f} | ML F1: {res['ml_model']['f1_mean']:.4f}")
    return results
