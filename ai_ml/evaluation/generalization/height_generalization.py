"""
Milestone 4 Evaluation Script: Height Generalization
Evaluates baseline_v1.0_frozen vs PointNet2_MSG_DualHead_v1 across Height Generalization conditions.
"""
...
def evaluate_height():
    print("Running Height Generalization & Identical-Height Stress Test...")
    ...
    gen = ExtendedSceneGenerator(seed=4200)

    height_pairs = [[4.0, 4.0], [5.0, 11.0], [7.0, 13.0], [8.0, 17.0], [10.0, 20.0], [6.0, 6.0]]
    results = {}
    for h_pair in height_pairs:
        key = f"{h_pair[0]}m_{h_pair[1]}m"
        res = run_eval(device, ml_model, decoder, extractor_baseline, gen, num_trials=10, heights=h_pair, setback_m=1.5)
        results[key] = res
        print(f"  Heights {key:12s} | Base F1: {res['baseline']['f1_mean']:.4f} (Merge {res['baseline']['merge_mean']:.1f}%) | ML F1: {res['ml_model']['f1_mean']:.4f} (Merge {res['ml_model']['merge_mean']:.1f}%)")
    return results
